...
InterfaceElem1d :: computeStressVector(FloatArray &answer, const FloatArray &strain, GaussPoint *gp, TimeStep *tStep)
{
    setCoordMode();
    switch ( mode ) {
        case ie1d_1d: answer = Vec1(static_cast< StructuralInterfaceCrossSection* >(this->giveCrossSection())->giveEngTraction_n(strain.at(1), gp, tStep)); return;
        case ie1d_2d: answer = static_cast< StructuralInterfaceCrossSection* >(this->giveCrossSection())->giveEngTraction_nt(strain, gp, tStep); return;
        case ie1d_3d: answer = static_cast< StructuralInterfaceCrossSection* >(this->giveCrossSection())->giveEngTraction_ntt(strain, gp, tStep); return;
    }
}
...
InterfaceElem1d :: computeConstitutiveMatrixAt(FloatMatrix &answer, MatResponseMode rMode, GaussPoint *gp, TimeStep *tStep)
{
    setCoordMode();
    switch ( mode ) {
    case ie1d_1d: answer = static_cast< StructuralInterfaceCrossSection* >(this->giveCrossSection())->giveStiffnessMatrix_Eng_n(rMode, gp, tStep); return;
    case ie1d_2d: answer = static_cast< StructuralInterfaceCrossSection* >(this->giveCrossSection())->giveStiffnessMatrix_Eng_nt(rMode, gp, tStep); return;
    case ie1d_3d: answer = static_cast< StructuralInterfaceCrossSection* >(this->giveCrossSection())->giveStiffnessMatrix_Eng_ntt(rMode, gp, tStep); return;
    }
}
